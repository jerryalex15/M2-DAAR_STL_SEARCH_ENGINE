...
\section{Tests de performance et résultats}
    ...
    lorem upsum feisfg hgfbegjhbqdjfhqbsf

    Ajouter ici les Diagramme de testbeds

    blablabalbalbalbalablababalabalbalbablala

    bodbdobdobdodbodbdobdobdob


\section{Discussion des résultats}
Avoir un moteur de recherche qui répond dans les meilleurs délais possibles est toujours l’idéal. Lors de nos tests de performance, nous avons observé que nos algorithmes de recherche retournent les résultats quasiment instantanément, avec le temps de réponse le plus long inférieur à 281,42 millisecondes. Cette rapidité est en grande partie due au choix de stockage de la table d’index, qui est un objet sérialisé en Python $(.pkl)$ sous forme de dictionnaire \textit{(hashmap)}. Cet objet permet d’obtenir en temps constant $O(1)$ les listes de livres associées à chaque terme recherché. De plus, d’autres objets, tels que la matrice d’adjacence et les indices de centralité, sont pré-calculés pour éviter de les recalculer à chaque recherche. Il suffit simplement de charger ces objets lors de chaque requête.
